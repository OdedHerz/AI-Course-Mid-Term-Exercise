...
|---------------|--------------------|-----------
| Chunk Size    | 300 chars          | Balances precision with context 
| Overlap       | 40 chars           | Prevents mid-word cuts 
...
**How It Works**:
1. Split each page into 300-character chunks with 40-char overlap
2. Store chunks with embeddings in Supabase
3. On query: Retrieve top-6 similar chunks
4. If ≥3 chunks from same parent page → Add full parent for context
5. Pass chunks (+ parent if merged) to LLM
...
- ⚠️ May lack context (solved by auto-merge)
...
- Small = Precision ✓, Less context ⚠️
- Large = More context ✓, Lower precision ⚠️
- **Choice**: Balance for insurance documents
